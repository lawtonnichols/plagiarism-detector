...
object RemoveEmptyDecl extends JSStatelessTransformPass {
  val statelessTransformer: PartialFunction[JSAST, JSAST] = 
    { case d@JSDecl(bindings) if bindings.isEmpty => val res = JSUndef(); res.lineno = d.lineno; res }
}
...
object HoistFunctions extends JSOnlyUpwardPass[List[JSFunctionDecl]] {
  ...
  val functionDeclTransform: Transformer =
    { case (f@JSFunctionDecl(name, params, body), _) =>
      val fundecl = JSFunctionDecl(name, params, newBody(body))
      fundecl.lineno = f.lineno
        (JSUndef(),
         List(fundecl))
   }
  ...
  val functionExpTransform: Transformer =
    { case (f@JSFunctionExp(name, params, body), _) =>
        val funexp = JSFunctionExp(name, params, newBody(body))
        funexp.lineno = f.lineno
        (funexp, upwardBase) }
...
object HoistFunctionVars extends JSOnlyUpwardPass[Set[JSPVar]] {
    ...
    { case (TransformDecl(bindings, retval), _) =>
        for {
          ...
          retvalAst <- transformExp(retval, downwardBase)
            ...
            (cur, res) => {
              val simpleassign = JSSimpleAssign(cur._1, cur._2)
              simpleassign.lineno = cur._2.lineno
              val binop = JSBinop(simpleassign, JSComma, res)
              binop.lineno = simpleassign.lineno
              binop
            })
   }
    ...
    { case (fe@JSFunctionExp(name, params, body), _) => {
        ...
        name match {
          case Some(n) => {
            ...
            val simpleassign = JSSimpleAssign(newVar,
                            makeFunction(List((n, Some(newVar)))))
            simpleassign.lineno= fe.lineno
            (simpleassign,
             Set(newVar))
          }
          case None =>
  ...
  // to get a handle on the function scope
  val functionDeclTransform: Transformer =
    { case (fd@JSFunctionDecl(name, params, body), _) => {
        val (bodyAst, nestedToHoist) = transformStmt(body, downwardBase)
        val functiondecl = JSFunctionDecl(name,
                        params,
                        JSSeq(List(
                          JSDecl(nestedToHoist.toList.map((_, Some(JSUndef())))),
                          bodyAst)))
        functiondecl.lineno = fd.lineno
        (functiondecl, Set(name))
    }
   }
...
object HoistGlobalVars extends JSOnlyDownwardPass[List[Set[JSPVar]]] {
  ...
  val toplevelDeclTransform: Transformer =
    { case (d@JSToplevelDecl(vars, rest), scope) =>
        for {
          ast <- transformStmt(rest, vars :: scope)
        } yield {
          val res = JSToplevelDecl(vars, ast)
          res.lineno = d.lineno
          res
        }
   }
  ...
  val simpleAssignTransform: Transformer =
    { case (sa@JSSimpleAssign(x@JSPVar(name), rhs), scope) if !isInScope(x, scope) =>
        for {
          rhsAst <- transformExp(rhs, scope)
        } yield {
          val res = updateGlobally(name, rhsAst)
          res.lineno = sa.lineno
          res
        }
   }
  ...
  val tryTransform: Transformer =
    { case (t@JSTry(body, catchPart, finallyPart), scope) =>
      val jstry = JSTry(transformStmt(body, scope)._1,
               catchPart.map(
                 { case (x, s) => (x, transformStmt(s, Set(x) :: scope)._1) }),
               finallyPart.map(s => transformStmt(s, scope)._1))
      jstry.lineno = t.lineno
      (jstry, upwardBase) }
  ...
  val varTransform: Transformer =
    { case (x@JSPVar(name), scope) if !isInScope(x, scope) =>
        val access = JSAccess(JSPVar(windowName), JSStr(name))
        access.lineno = x.lineno
        (access, upwardBase) }
...
object FixContinueLabels extends JSStatelessTransformPass {
  ...
  val continueTransformer: Transformer =
    { case c@JSContinue(Some(JSLabel(label))) => 
        val res = JSContinue(Some(contLbl(label))) 
        res.lineno = c.lineno
        res
    }

  val loopLabelTransformer: Transformer =
    { case ls@JSLabeledStmt(labels, loop: JSLoop) if labels.size >= 1 => {
        if (labels.size != 1) {
      ...
      newLoop.lineno = loop.lineno
      val res = JSLabeledStmt(labels, newLoop)
      res.lineno = ls.lineno
      res
    }
   }
...
object MakeAllAssignmentsSimple extends JSStatelessTransformPass {
  ...
  val assignTransform: Transformer =
    { case a@JSCompoundAssign(x, op, rhs) => 
	       val res = JSSimpleAssign(x, JSBinop(x, op, transformExp(rhs))) 
         res.lineno = a.lineno
         res
    }

  val updateTransform: Transformer =
    { case c@JSCompoundUpdate(a@JSAccess(target, field), op, rhs) => {
	val y1 = freshScratch
	val y2 = freshScratch
	val access = JSAccess(y1, y2)
  access.lineno = a.lineno
  val simpleupdate = JSSimpleUpdate(access, JSBinop(access, op, transformExp(rhs)))
  simpleupdate.lineno = c.lineno
  y1.lineno = c.lineno
  y2.lineno = c.lineno
	val res = TransformDecl(
...
object FunctionDeclToExpInFunction extends JSStatelessTransformPass {
  ...
  val statelessTransformer: Transformer =
    { case fd@JSFunctionDecl(name, params, body) => {
	val newFunc = JSFunctionExp(Some(name),
				    params,
				    transformStmt(body))
  newFunc.lineno = fd.lineno
	val res = JSSimpleAssign(name, newFunc)
  res.lineno = fd.lineno
  res
    }
   }
}
...
object FunctionDeclToExp extends JSStatelessTransformPass {
  ...
  val functionDeclTransformer: Transformer =
    { case fd@JSFunctionDecl(name, params, body) =>
      val funexp = JSFunctionExp(Some(name),
             params,
             FunctionDeclToExpInFunction.transformStmt(body))
      funexp.lineno = fd.lineno
	     val res = updateGlobally(name.name,
		       funexp) 
       res.lineno = fd.lineno
       res
     }
  ...
  val functionExpTransformer: Transformer =
    { case fe@JSFunctionExp(name, params, body) => 
        val res = JSFunctionExp(name, params,
                      FunctionDeclToExpInFunction.transformStmt(body)) 
        res.lineno = fe.lineno
        res
    }
...
object RemoveThis extends JSStatelessTransformPass {
  ...
  val thisTransformer: Transformer =
    { case t@JSThis() => val res = JSPVar(windowName); res.lineno = t.lineno; res }

  val functionExpTransformer: Transformer =
    { case fe@JSFunctionExp(name, params, body) =>
	val res = JSFunctionExp(name,
		      params,
...
object HandleCatchScoping extends JSTransformPass[Map[JSPVar, JSPVar], Set[JSPVar]] {
  ...
  val tryTransformer: Transformer =
    { case (t@JSTry(body, catchPart, finallyPart), varMap) => {
        ...
        // newCatchPart.lineno = catchPart.lineno
        val newFinallyPart =
          finallyPart.map(f => transformStmt(f, varMap))
        // newFinallyPart.lineno = finallyPart.lineno
        val jstry = JSTry(bodyAst,
  ...
  val realFunctionExpTransformer: Transformer =
    { case (fe@JSFunctionExp(name, params, s@JSSeq(d@JSDecl(bindings) :: rest)), varMap) => {
        ...
        val decl = JSDecl(newBindings)
        val seq = JSSeq(decl :: restAsts)
        seq.lineno = s.lineno
        val funexp = JSFunctionExp(name, params, seq)
        funexp.lineno = fe.lineno
        (funexp, upwardBase)
    }
   }
